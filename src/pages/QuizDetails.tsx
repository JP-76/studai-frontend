...
function QuizDetails() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base-200">
        <div className="text-center">
          ...
          <button className="btn btn-primary" onClick={() => navigate("/home")}>Voltar ao início</button>
        </div>
      </div>
    );
  }
  ...
  function exportAttemptsToCSV() {
    ...
    link.href = url;
    link.setAttribute("download", `quiz_${quiz ? sanitizeFilename(quiz.title) : 'quiz'}_resultados.csv`);
  ...
  function exportAttemptsToXLSX() {
    ...
    XLSX.writeFile(workbook, `quiz_${quiz ? sanitizeFilename(quiz.title) : 'quiz'}_resultados.xlsx`);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="card bg-base-100 shadow-xl mb-6">
          <div className="card-body">
            ...
            <div className="mb-2 text-sm text-base-content/60">
              <span className="font-semibold">Conteúdo da Fonte:</span> {quiz.sourceContent}
            </div>
            <div className="mb-2 text-sm text-base-content/60">
              <span className="font-semibold">Idioma:</span> {quiz.languageCode}
            </div>
            <div className="mb-2 text-sm text-base-content/60">
              <span className="font-semibold">Criado em:</span> {new Date(quiz.createdAt).toLocaleString()}
            </div>
            <div className="mb-2 text-sm text-base-content/60">
              <span className="font-semibold">Atualizado em:</span> {new Date(quiz.updatedAt).toLocaleString()}
            </div>
            <div className="mb-2 text-sm text-base-content/60">
              <span className="font-semibold">Total de Questões:</span> {quiz.questions.length}
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex justify-between items-center">
          <div className="flex gap-2">
            <button
              ...
              onClick={() => {
                navigator.clipboard.writeText(`${window.location.origin}/quiz/${quiz.id}/visitor`);
        ...
        {showExportModal && (
          <div className="modal modal-open">
            ...
            <div className="modal-backdrop" onClick={() => setShowExportModal(false)}></div>
          </div>
        )}
        {/* Attempts List */}
        <div className="card bg-base-100 shadow-xl">
          <div className="card-body">
            ...
            {attempts.length === 0 ? (
              <p className="text-base-content/70">Nenhuma tentativa registrada ainda.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="table w-full">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th onClick={() => handleSort('guestUser')} className="cursor-pointer select-none">Usuário {sortConfig.key === 'guestUser' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}</th>
                      <th onClick={() => handleSort('score')} className="cursor-pointer select-none">Pontuação {sortConfig.key === 'score' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}</th>
                      <th onClick={() => handleSort('timeSpent')} className="cursor-pointer select-none">Tempo Gasto {sortConfig.key === 'timeSpent' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}</th>
                      <th onClick={() => handleSort('createdAt')} className="cursor-pointer select-none">Data {sortConfig.key === 'createdAt' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sortedAttempts.map((attempt, idx) => (
                      <tr key={attempt.id}>
                        <td>{idx + 1}</td>
                        <td>{attempt.guestUser ? (attempt.guestName || "Visitante") : "Você"}</td>
                        <td>{attempt.score}%</td>
                        <td>{Math.round(attempt.timeSpent / 60)}m {attempt.timeSpent % 60}s</td>
